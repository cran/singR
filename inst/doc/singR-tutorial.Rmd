...
The singR package is built upon the SING method <https://github.com/thebrisklab/SING>, which is designed to extract joint and individual non-Gaussian components from multiple datasets. This package provides a tutorial example to support the paper:

singR: An R Package for Simultaneous Non-Gaussian Component Analysis for Data Integration <https://journal.r-project.org/articles/RJ-2023-084/>.

In this paper, two examples are provided. The dataset for the first example is included within this R package, while the dataset for the second example is available at the following link:
<https://github.com/thebrisklab/singR/tree/main/extdata>.

## Installation

You can install singR from github or CRAN with:
```{r,eval=FALSE}
library(devtools)
